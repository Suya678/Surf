...
app.post("/api/listing/create", isAuthenticated, async (req, res) => {
  ...
  // --- Validation ---
  if (!title || !address || !city || !province || !postal_code) {
    return res.status(400).json({
      error:
        "title, address, city, province, and postal_code are required.",
    });
  }
  ...
  try {
    ...
    res.status(201).json(listingResult.rows[0]); // 201 Created
  } catch (err) {
...
app.get("/api/listings/search", async (req, res) => {
  ...
  try {
    centerCoordinates = await getCoordinates(address, city, province, postal_code);
